@extends('layouts.admin')

@section('content')
<div class="row">
    <div class="col-md-12 col-sm-12 col-xs-12">
        <div class="x_panel">
            <div class="x_title">
                <h2>Create New Tax</h2>
                <div class="clearfix"></div>
            </div>
            <div class="x_content">
                <form method="post" id="formCreate" class="form-horizontal">
                    <div id="demo-form2" class="form-horizontal form-label-left">
                        <!--<div class="form-group">
                            <label class="control-label col-md-3 col-sm-3 col-xs-12">Tax
                            </label>
                            <div class="col-md-6 col-sm-6 col-xs-12">
                                <select name="tax" class="form-control col-md-7 col-xs-12 selecttax">
                                    <option value="SINGLE">Single</option>
                                    <option value="GROUP">Group</option>
                                </select>
                            </div>
                        </div>-->
                        <div class="form-group">
                            <label class="control-label col-md-3 col-sm-3 col-xs-12">Name
                            </label>
                            <div class="col-md-6 col-sm-6 col-xs-12">
                                <input type="text" name="name" class="form-control col-md-7 col-xs-12">
                            </div>
                        </div>
                        <div class="form-group">
                            <label class="control-label col-md-3 col-sm-3 col-xs-12">Effective Rate
                            </label>
                            <div class="col-md-6 col-sm-6 col-xs-12">
                                <input type="number" name="effective_rate" class="form-control col-md-7 col-xs-12">
                            </div>
                        </div>
                        <!--<div class="form-group">
                            <label class="control-label col-md-3 col-sm-3 col-xs-12">Witholding
                            </label>
                            <div class="col-md-6 col-sm-6 col-xs-12">
                                <div class="checkbox">
                                    <label>
                                        <input type="checkbox" class="flat" name="iCheck" name="witholding" value="1">
                                    </label>
                                </div>
                            </div>
                        </div>-->
                        <div class="form-group">
                            <label class="control-label col-md-3 col-sm-3 col-xs-12">Sell Tax Account
                            </label>
                            <div class="col-md-6 col-sm-6 col-xs-12">
                                <select name="sell_tax_account" class="form-control col-md-7 col-xs-12 selecttax">
                                    @foreach($coa as $c)
                                    <option value="{{$c->id}}" @if($c->id == 50) selected @endif>({{$c->code}}) - {{$c->name}} ({{$c->coa_category->name}})</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                        <div class="form-group">
                            <label class="control-label col-md-3 col-sm-3 col-xs-12">Buy Tax Account
                            </label>
                            <div class="col-md-6 col-sm-6 col-xs-12">
                                <select name="buy_tax_account" class="form-control col-md-7 col-xs-12 selecttax">
                                    @foreach($coa2 as $c)
                                    <option value="{{$c->id}}" @if($c->id == 14) selected @endif>({{$c->code}}) - {{$c->name}} ({{$c->coa_category->name}})</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                    </div>
                    <div class="ln_solid"></div>
                    <div class="col-md-12 text-center">
                        <div class="form-group">
                            <a href="{{ url('/other/taxes') }}" class="btn btn-primary">Cancel</a>
                            <button type="button" id="click" class="btn btn-success">Create</button>
                        </div>
                    </div>
                </form>
            </div>
        </div>
    </div>
</div>
@endsection

@push('scripts')
<script src="{{asset('js/otherlists/taxes/createForm.js?v=5-20200305-1546') }}" charset="utf-8"></script>
<script src="{{asset('js/other/select2.js?v=5-20200305-1546') }}" charset="utf-8"></script>
<script src="{{asset('js/other/zebradatepicker.js?v=5-20200305-1546') }}" charset="utf-8"></script>
@endpush