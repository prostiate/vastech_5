...
class CheckSalesPaymentStatus extends Command
{
    ...
    {
        $date               = Carbon::now();
        $header             = sale_payment::where('status', '<>', 2)->whereDate('due_date', '<',$date)->get();
        $other_transactions = other_transaction::where('type', 'sales payment')->where('status', '<>', 2)->whereDate('due_date', '<', $date)->get();
