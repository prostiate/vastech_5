...
class PurchaseReturnController extends Controller
{
        ...
        if ($user->company_id == 5) {
            ...
            if (isset($number)) {
                if ($dt->isSameMonth($number->transaction_date)) {
                    $trans_no       = now()->format('m') . '/' . now()->format('y') . '.' . $number1 . '.PR';
                } else {
                    $check_number   = purchase_return::whereMonth('transaction_date', Carbon::parse($dt))->latest()->first();
                    if ($check_number) {
                        $trans_no   = now()->format('m') . '/' . now()->format('y') . '.' . $number1 . '.PR';
                    } else {
                        $number1    = 10001;
    ...
    public function store(Request $request)
    {
        ...
        if ($user->company_id == 5) {
            ...
            $number1            = $misahy[1] + 1;
            if (isset($number)) {
                    $check_number   = purchase_return::whereMonth('transaction_date', Carbon::parse($request->trans_date))->latest()->first();
                    if ($check_number) {
                        ...
                        $number2    = $misahy[1] + 1;
                        $trans_no   = Carbon::parse($request->trans_date)->format('m') . '/' . Carbon::parse($request->trans_date)->format('y') . '.' . $number2 . '.PR';
                    } else {
                        $number1    = 10001;
                        $trans_no   = Carbon::parse($request->trans_date)->format('m') . '/' . Carbon::parse($request->trans_date)->format('y') . '.' . $number1 . '.PR';
                    }
            } else {
                $check_number   = purchase_return::whereMonth('transaction_date', Carbon::parse($request->trans_date))->latest()->first();
                if ($check_number) {
    ...
    public function update(Request $request)
    {
        ...
        try {
            ...
            other_transaction::where('type', 'purchase return')->where('number', $pi->number)->update([
                'transaction_date'              => $request->get('return_date'),
                'memo'                          => $request->get('memo'),
            ]);
            $pi->update([
            ...
            $ambilot                            = other_transaction::where('number', $pi->transaction_no_pi)->where('type', 'purchase invoice')->first();
            $ambilot->update([
                'balance_due'                   => $ambilpi->balance_due,
            ]);
            if ($ambilpi->balance_due == 0) {
            ...
            foreach ($request->products as $i => $keys) {
                if ($request->qty[$i] > 0) {
                    ...
                    $pi->purchase_return_item()->save($pp[$i]);
                    // UPDATE ITEM INVOICE
                    if ($request->qty[$i] > $request->qty_remaining_return[$i]) {
                        DB::rollBack();
                        return response()->json(['errors' => 'Quantity cannot be more than order']);
