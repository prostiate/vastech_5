...
class WipController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $trans_no = $number + 1;
        $rules = array(
            'wip_product_id'   => 'required|array|min:1',
            'wip_product_id.*' => 'required',
        );
        ...
        DB::beginTransaction();
        try {
            $product                            = $request->wip_product_id;
            ...
            $transactions                       = other_transaction::create([
                ...
                'status'                        => 2,
                'balance_due'                   => 0,
                'total'                         => ($request->grandtotal_without_qty * $request->product_qty) + $request->margin_total,
            ]);
            $transactions->save();
            ...
            $header                             = wip::create([
                ...
                'grandtotal_without_qty'        => $request->grandtotal_without_qty,
                'grandtotal_with_qty'           => $request->grandtotal_with_qty,
            ]);
            $header->save();
            ...
            $price                              = $request->grandtotal_without_qty + $request->margin_total;
            ...
            coa_detail::create([
                ...
                'number'                        => 'WIP #' . $trans_no,
                'contact_id'                    => $request->contact,
                'debit'                         => (($request->grandtotal_without_qty * $request->product_qty) + $request->margin_total),
                'credit'                        => 0,
            ]);
            $get_current_balance_on_coa         = coa::find($produk->default_inventory_account);
            coa::find($get_current_balance_on_coa->id)->update([
                'balance'                       => $get_current_balance_on_coa->balance + (($request->grandtotal_without_qty * $request->product_qty) + $request->margin_total),
            ]);
            // BIKIN COA DETAIL DAN BALANCE BUAT MARGIN
            if ($request->margin_total > 0) {
                coa_detail::create([
                    ...
                    'contact_id'                => $request->contact,
                    'debit'                     => 0,
                    'credit'                    => $request->margin_total,
                ]);
                $get_current_balance_on_coa     = coa::find(74); // COST OF PRODUCTION LANGSUNG DARI COA
                coa::find($get_current_balance_on_coa->id)->update([
                    'balance'                   => $get_current_balance_on_coa->balance + $request->margin_total,
                ]);
            }

            foreach ($product as $i => $p) {
                $item[$i]                       = new wip_item([
                    'product_id'                => $request->wip_product_id[$i],
                    //'qty_in_stock'              => $request->wip_product_qty_in_stock[$i],
                    'qty_require'               => $request->wip_product_req_qty[$i],
                    'qty_total'                 => $request->wip_product_req_qty[$i] * $request->product_qty,
                    'price'                     => $request->wip_product_price[$i],
                    'total_price'               => $request->wip_product_total_price[$i],
                ]);
                ...
                $wh->type                       = 'wip';
                $wh->number                     = 'WIP #' . $trans_no;
                $wh->product_id                 = $request->wip_product_id[$i];
                $wh->warehouse_id               = $request->warehouse;
                $wh->qty_out                    = $request->wip_product_req_qty[$i] * $request->product_qty;
                $wh->save();
                //merubah harga average produk
                $produk                         = product::find($request->wip_product_id[$i]);
                $qty                            = $request->wip_product_req_qty[$i] * $request->product_qty;
                //menyimpan jumlah perubahan pada produk
                product::where('id', $request->wip_product_id[$i])->update([
                    'qty'                       => $produk->qty - $qty,
                ]);
                // BIKIN COA DETAIL DAN BALANCE BUAT PER BARANG RAW (MASUK KE INVENTORY CREDIT)
                $default_product_account        = product::find($request->wip_product_id[$i]);
                coa_detail::create([
                    ...
                    'contact_id'                => $request->contact,
                    'debit'                     => 0,
                    'credit'                    => ($request->wip_product_total_price[$i] * $request->product_qty),
                ]);
                $get_current_balance_on_coa     = coa::find($default_product_account->default_inventory_account);
                coa::find($get_current_balance_on_coa->id)->update([
                    'balance'                   => $get_current_balance_on_coa->balance - ($request->wip_product_total_price[$i] * $request->product_qty),
                ]);
            }
            DB::commit();
    ...
    public function show($id)
    {
        ...
        return view('admin.request.sukses.wip.show', compact(['wip', 'wip_item', 'quantity_in_stock', 'get_all_detail', 'total_debit', 'total_credit']));
    }
